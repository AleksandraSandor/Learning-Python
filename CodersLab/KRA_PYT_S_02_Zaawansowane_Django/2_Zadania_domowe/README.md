...
* Zdefiniuj szablon bazowy, po którym dziedziczą wszystkie inne szablony w aplikacji,
...
* Napisz formularz dodawania kategorii produktów do bazy danych:
    * lista kategorii: zmodyfikuj widok `/categories`: przy opisie każdej kategorii, dodaj link **modyfikuj**, na dole listy kategorii dodaj link **dodaj kategorię**,
    * edycja kategorii: po wejściu na link do edycji wyświetla się fomrularz edycji kategorii. Można zmienić dane i je zapisać. 
    * dodawanie nowej kategorii: po wejściu na link **dodaj kategorię**, powinien wyświetlić się pusty formularz, w którym można dodać nową kategorię i ją zapisać.

### Zadanie 2.

* Napisz formularz dodawania towarów do bazy danych. Dodaj widoki: 
    * lista produktów: wyświetla listę **wszystkich** produktów, niezależnie od kategorii (możesz w tym celu zmodyfikować widok `/category`. Przy opisie każdego progduktu powinien być link do edycji. Parametr powinien być przekazywany w URL-u. Na dole listy osób powinien być link **dodaj produkt**.
    * edycja produktu: po wejściu na link do edycji wyświetla się formularz edycji produktu. Można zmienić dane i je zapisać.
    * dodawanie produktu: po wejściu w link **dodaj produkt**, powinien wyświetlić się pusty formularz, w którym można dodać nowy towar i go zapisać.
...
* Zabezpiecz aplikację w taki sposób, by widoki edycji towaru i kategorii były dostępne tylko dla użytkowników, kótrzy mają przyznane prawa do odpowiednich akcji: "add\_product", "add\_category", "modify\_product", itp. do odpowiednich modeli.
...
* Wygeneruj odpowiednie klasy admina dla modeli użytych w pracy domowej. Zapewnij czytelność przez odpowiednie nadpisanie metody `__str__()`. Zapewnij łatwość użytkowania dodając odpowiednie pola do listy modeli.
